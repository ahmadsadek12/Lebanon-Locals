...
(function() {
    ...
    async function loadReviews() {
        try {
            // Map frontend type to database reviewee_type
            // experience and event are both stored as 'hosting' in the reviews table
            let dbRevieweeType = revieweeType;
            if (revieweeType === 'experience' || revieweeType === 'event') {
                dbRevieweeType = 'hosting';
            }

            // Use relative path to backend API
            const apiUrl = `/backend/api/reviews.php?reviewee_type=${dbRevieweeType}&reviewee_id=${revieweeId}`;
    ...
    /**
     * Render star rating - simple version like details page
     */

...
try {
    ...
    if (!isset($input['name']) || empty(trim($input['name']))) {
        ...
        echo json_encode(['success' => false, 'message' => 'Subtype name is required']);
        exit();
    }
    ...
    $insertStmt = $db->prepare("
        INSERT INTO subtypes (name, category, description, icon_url)
        VALUES (:name, :category, :description, :icon_url)
    ");
    ...
    if (isset($_FILES['icon_file']) && $_FILES['icon_file']['error'] === UPLOAD_ERR_OK) {
        ...
        $updateIconStmt = $db->prepare("UPDATE subtypes SET icon_url = :icon_url WHERE id = :id");
